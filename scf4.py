...
cp = plt.contourf(Xp, Yp, Z_final_pot, levels=levels, cmap=plt.cm.plasma)
...
cp_QM = plt.contourf(Xfd, Yfd, ZCharge, levels=levels_c, cmap=plt.cm.plasma)
plt.colorbar(cp_QM)
plt.xlabel('X [nm]')
plt.ylabel('Y [nm]')
...
for cur_valley in range(0, 3):
	...
	for row in range(0,div):
		for col in range(0,ncols):
			...
			axarr[row, col].contourf(X, Y, plotZ, levels=levels, cmap=plt.cm.plasma)
